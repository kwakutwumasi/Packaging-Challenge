...
public class PackerFileParserImpl implements PackerFileParser {
	...
	private void processLine(String line, List<PackerFileEntry> entries) throws APIException {
		String[] lineParts = line.split("\\s+:\\s+",2);
		Double weightLimit; 
		try {
			weightLimit = Double.parseDouble(lineParts[0]);
		} catch (NumberFormatException e) {
			throw new APIException(MessageFormat.format(Messages.get("invalid.weight.limit"), lineParts[0]));
		}
		
		if(lineParts.length!=2)
			throw new APIException(MessageFormat.format(Messages.get("invalid.line"), line));
		
		PackerFileEntry entry = new PackerFileEntry().withWeightLimitAs(weightLimit);
		entries.add(entry);
		processEntries(entry, lineParts[2]);
	}

	private void processEntries(PackerFileEntry entry, String itemsString) throws APIException {
		String[] items = itemsString.split("\\s+");
		for(String item:items) {
			if(item.startsWith("(")
					&& item.endsWith(")")) {				
				processEntry(entry,item.substring(1,item.length()-1));
			} else {
				throw new APIException(MessageFormat.format(Messages.get("invalid.item"), item));
			}
		}
	}

	private void processEntry(PackerFileEntry entry, String item) throws APIException {
		...
		try {
			...
			throw new APIException(MessageFormat.format(Messages.get("invalid.index"), itemParts[0]));
		}
		...
		try {
			...
			throw new APIException(MessageFormat.format(Messages.get("invalid.weight"), itemParts[1]));
		}

		if(itemParts[2].length()<2) {
			throw new APIException(MessageFormat.format(Messages.get("invalid.cost"), itemParts[2]));
		}
			
		String currency = itemParts[2].substring(0, 1);
		
		if(!currencyMap.containsKey(currency))
			throw new APIException(MessageFormat.format(Messages.get("invalid.currency"), currency));
		...
		try {
			...
			throw new APIException(MessageFormat.format(Messages.get("invalid.cost"), costString));
		}
